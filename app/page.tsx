...
export default function Home() {
  ...
  const getAIResponse = (query: string): string => {
    ...
    if (q.includes("mulai") || q.includes("bi system") || q.includes("business intelligence") || q.includes("invoice") || q.includes("receipt")) {
      return "The Mulai BI System is an autonomous, stateful business intelligence pipeline powered by LangGraph. It ingests raw enterprise invoices and unstructured receipts, coordinates vision-enabled extractors (Gemini 1.5 Flash) to construct structured schemas, performs mathematical outlier and pricing anomaly detection on regional pricing histories, and persists data securely to PostgreSQL with a SQLite fallback layer. It's optimized for enterprise-scale document analysis.";
    }
    if (q.includes("repo") || q.includes("analyzer") || q.includes("github repository analyzer") || q.includes("github analyzer") || q.includes("code analyzer")) {
      return "The GitHub Repository Analyzer is an AI-powered codebase intelligence platform. I designed a custom RAG (Retrieval-Augmented Generation) pipeline using LangChain, ChromaDB (vector store), and Jina Embeddings to support interactive codebase Q&A. To handle long-running operations and repository downloads asynchronously without blocking the UI, I set up a distributed task worker queue using Celery and Redis, running in Docker container environments.";
    }
    if (q.includes("skillsync") || q.includes("skill sync") || q.includes("learning analytics") || q.includes("saas")) {
      return "SkillSync is a learning analytics SaaS tracking practice streaks and progression. I built a rapid FastAPI backend in Python, utilizing SQLAlchemy ORM and PostgreSQL connection pooling to calculate activity metrics, streaks, and user history. On the frontend, I developed a high-fidelity React and TypeScript UI using Zustand for lightweight state management and Axios for secure, JWT-authenticated transactional API calls.";
    }
    if (q.includes("bookmyshow") || q.includes("book my show") || q.includes("movie booking") || q.includes("ticket booking")) {
      return "The BookMyShow Clone is a high-fidelity ticket booking application replicating complete, end-to-end live movie checkout flows. I designed it to manage complex client-side seat reservation grids, multi-step booking pipelines (seat selection -> verification -> checkout), state persistence, and simulated payment integrations, providing a responsive and fluid desktop/mobile checkout interface.";
    }
    if (q.includes("project") || q.includes("portfolio")) {
      return "I have engineered four flagship production-grade systems. Ask me about any of these to see the architecture details:\n\n1. Mulai BI System: Stateful LangGraph AI pipeline parsing unstructured invoices.\n2. GitHub Repository Analyzer: Asynchronous codebase Q&A using LangChain, Celery, and Redis.\n3. SkillSync: Learning analytics SaaS built with FastAPI, React, and Zustand.\n4. BookMyShow Clone: Multi-stage movie ticket transactional grid.";
    }
